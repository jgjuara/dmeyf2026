# Desafio Cazatalentos Ordenamiento — pueblo con p heterogeneos
# Misma estructura que simular_desafio.R. Cambia el supuesto de base:
# las jugadoras de un pueblo no comparten un unico p.
#
# Ci < Cj  sii  P( encestes_Ci < encestes_Cj | 100 tiros ) > 0.5
#
# Supuestos:
# - Cada pueblo tiene un centro mu ~ Unif[0, P_MAX), analogo a generar_p()
#   del script con p comun. Las n adolescentes de ese pueblo tienen
#   p_i ~ N(mu, SIGMA) truncada a (0, P_MAX). Taurasi (0.85) sigue siendo
#   estrictamente superior.
# - SIGMA = 0.02 es el desvio del peloton normal de CazaTalentos.R.
# - Se guarda el p de la candidata (la elegida por el procedimiento), no mu.
...
# - En C5 el estadistico suficiente es la suma 701 encestes en 1000 tiros.
...
# n jugadoras, posiblemente con p distintos; qty tiros independientes cada una.
# rbinom(n, qty, probs) es la misma ley que mapply(ftirar, probs, qty).
...
generar_pueblo <- function(n) {
  mu <- runif(1, min = 0, max = P_MAX)
  rnorm_trunc(n, mu, SIGMA, lo = 0, hi = P_MAX)
}

# recolecta N_ACEPTADOS valores de p de la candidata que pasan el filtro
...
sim_c1 <- function() {
  jugadoras <- generar_pueblo(100)
  vaciertos <- encestes_grupo(jugadoras, TIROS)
...
sim_c3 <- function() {
  jugadoras <- generar_pueblo(100)
  candidata <- jugadoras[13]
  aciertos <- ftirar(candidata, TIROS)
  if (aciertos != 80) {
    return(NA_real_)
  }
  candidata
...
sim_c5 <- function() {
  candidata <- generar_pueblo(1)
  rondas <- encestes_grupo(rep(candidata, 10), TIROS)
  if (sum(rondas) != 701) {
    return(NA_real_)
  }
  candidata
...
# Nueva ronda en la gran ciudad: 100 tiros de cada candidata (su p individual)
nueva_ronda <- list()
