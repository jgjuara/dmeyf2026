...
require("here")
here::i_am("juara/jueves/z494/01/3_estudio_hiperparametros_auc.R")
...
for (hp in hiperparams) {
  ...
  xlim_bo <- c(as.numeric(hp_bounds$lower), as.numeric(hp_bounds$upper))
...
for (hp in hiperparams) {
  ...
  ylim_bo <- c(as.numeric(hp_bounds$lower), as.numeric(hp_bounds$upper))
